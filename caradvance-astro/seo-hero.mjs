...
// (A) .inner felső margó minimalizálása 12px-re minden oldalon (.hero-s oldalak)
const REPS = [
  ['max-width:1120px;margin:64px auto 0', 'max-width:1120px;margin:12px auto 0'],
  ['max-width:1120px;margin:24px auto 0', 'max-width:1120px;margin:12px auto 0'],
];
...
try {
  ...
  for (const f of walk('dist')) {
    let h = fs.readFileSync(f, 'utf8'), ch = false;
    for (const [a, b] of REPS) { if (h.includes(a)) { h = h.split(a).join(b); ch = true; } }
    if (ch) { fs.writeFileSync(f, h); n++; }
  }
  console.log('seo-hero: .inner felso terkoz minimalizalva ' + n + ' oldalon');
...
const NEST = 'border-radius:22px!important;padding:84px 18px 72px!important;min-height:480px!important;margin-left:-12px!important;margin-right:-12px!important;margin-bottom:8px!important;width:calc(100% + 24px)!important;max-width:none!important;box-sizing:border-box!important;';
...
inject('dist/uj-auto-berlese/index.html', 'cd-align-uab',
  '@media(max-width:640px){.egl-mhero{' + NEST + '}.egl-mhero-inner{margin-top:12px!important;}}' +
...
inject('dist/bizomanyos/index.html', 'cd-align-biz',
  '@media(max-width:640px){.bizhero{' + TOP + '}.bizhero-inner{margin-top:12px!important;}}'
);

// Kapcsolat: fészkelt kaphero
inject('dist/kapcsolat/index.html', 'cd-align-kap',
  '@media(max-width:640px){.kaphero{' + NEST + '}.kaphero-inner{margin-top:12px!important;}}'
);
